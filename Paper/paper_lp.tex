...
Symbol & Description\\
\midrule
\cellcolor{gray!10}{$M^{sl}(t)$} & \cellcolor{gray!10}{Total above- and belowground dry biomass per ha of AFS (t\,DM\,ha$^{-1}$) at stand age $t$}\\
$A(N)$ & Stand-level Gompertz asymptote (t\,DM\,ha$^{-1}$); density- and site-dependent\\
\cellcolor{gray!10}{$k$} & \cellcolor{gray!10}{Gompertz growth-rate coefficient (yr$^{-1}$)}\\
...
\cellcolor{gray!10}{$M^{ag}(t)$} & \cellcolor{gray!10}{Aboveground dry biomass per ha (t\,DM\,ha$^{-1}$) at age $t$}\\
$f_p(t)$ & asymtotic fraction of AGB attributed to compartment $p \in \{s,b,r\}$ at age $t$\\
\addlinespace
\cellcolor{gray!10}{$\alpha_p,\,\delta_p$} & \cellcolor{gray!10}{Intercept and slope of the linear fraction model for compartment $p$}\\
$q_p(t)$ & Merchantable fraction of compartment $p$ biomass at age $t$ (logistic)\\
\cellcolor{gray!10}{$r_p$} & \cellcolor{gray!10}{Logistic growth rate for merchantability of compartment $p$}\\
$t_{50,p}$ & Age at which 50\% of compartment $p$ biomass is merchantable (yr)\\
\cellcolor{gray!10}{$Y_p(t)$} & \cellcolor{gray!10}{Merchantable dry biomass of compartment $p$ per ha at harvest age $t$ (t\,DM\,ha$^{-1}$)}\\
\addlinespace
...
Combining equations \eqref{eq:gomp-stand}--\eqref{eq:logistic-merch}, the dry biomass yield of fraction \(p\) per hectare at harvest age \(t\) is: \begin{equation}
  ...
  \label{eq:yield}
\end{equation}
with all non-merchantable fractions aggregated into the residue fraction. Fresh-weight yields at felling are obtained from dry matter yields via \(\eta_p^{\mathrm{fw}}(t) = \eta_p(t)/(1-\omega)\), where the moisture content at felling \(\omega = 0.55\) is taken from \citet{Civitarese2019_Poplar_Harvest}. \autoref{fig:afs-growth-curves} shows the fractions' stacked biomass quantities \(\eta_p(t)\) depending on stand age compared with empirical observations from \citet{Jha2018_Poplar_AFS_Biomass}
...
{\centering \includegraphics[width=.9\textwidth]{figures/fig-results-sc-map-1} 
...
{\centering \includegraphics[width=\textwidth]{figures/fig-results-site-scatter-1} 
...
The base-case results presented in Section\textasciitilde\ref{sec:results} rest on a single
...
\subsection{Experimental Design}\label{sec:sens-design}

The sensitivity experiment follows a full-factorial design over four factors
(see commented scenario loop in code chunk \texttt{load-model-data}):

\begin{itemize}
\tightlist
\item
  \textbf{Opportunity cost level} (\(\bar{c}^{opp}\)) \(\in \{150, 300, 600\}\)\textasciitilde{}\euro/ha:
  the mean site-level opportunity cost is scaled uniformly, reflecting scenarios
  from subsidised or low-fertility land (150\textasciitilde{}\euro/ha) to fertile, high-price
  cereal-growing regions (600\textasciitilde{}\euro/ha).
\item
  \textbf{Logistics cost multiplier} (\(\lambda^{log}\)) \(\in \{0.5, 1.5, 2.0\}\):
...
\item
  \textbf{Establishment cost multiplier} (\(\lambda^{est}\)) \(\in \{0.5, 1.5, 2.0\}\):
...
\item
  \textbf{Revenue multiplier} (\(\lambda^{rev}\)) \(\in \{0.5, 0.75, 1.25\}\): a uniform
...
runs in total. The benchmark scenario uses \(\bar{c}^{opp} = 500\)\textasciitilde{}\euro/ha with
all multipliers set to 1.0. All runs use the same Gurobi solver with a
600-second time limit. Results are loaded from \texttt{res\_sens\_list\_lp\_v11.rds} and
post-processed via \texttt{extract\_site\_profit()}, which computes site-level KPIs
normalised to \euro/ha/yr for comparability across runs.

\subsection{Descriptive analysis of simulation results}\label{descriptive-analysis-of-simulation-results}
...
\caption{Distribution of site-level profit (\euro~ha$^{-1}$~yr$^{-1}$) across scenario parameter levels. Each panel row shows one cost dimension (logistics cost scaler \textit{cost.log}, top; establishment cost scaler \textit{cost.est}, bottom); columns show opportunity cost level (\textit{opp}, relative to baseline 150~\euro~ha$^{-1}$~yr$^{-1}$). Fill colour encodes the revenue scaler. Each box summarises all active sites across all 10 stochastic replications of that parameter combination. The dashed horizontal line marks zero profit.}\label{fig:fig-boxplot-scenario-profit}
\end{figure}
...
\caption{Distribution of total AFS area dedicated per scenario replicate (ha) across parameter levels. Each panel row shows one cost dimension (logistics cost scaler \textit{cost.log}, top; establishment cost scaler \textit{cost.est}, bottom); columns show opportunity cost level. Fill colour encodes the revenue scaler. Each box summarises the 10 stochastic replications of that parameter combination. Higher values indicate broader spatial activation of the AFS potential.}\label{fig:fig-boxplot-scenario-area}
\end{figure}

\subsection{Statistical Analysis: GAM-based effect estimation}\label{sec:sens-gam}
...
sensitivity data set \texttt{df\_sens}. GAMs are particularly appropriate here because
the AFS-SCD objective is a non-linear function of the cost and revenue parameters,
...
using cubic regression splines (\texttt{bs\ =\ "cr"}) fitted by restricted maximum
likelihood (REML). Basis dimensions are set conservatively (\(k \leq 5\)) given the
three-level discrete factor ranges; adequacy is verified via \texttt{mgcv::gam.check()}.
For the stem share a Beta family is specified to respect the unit-interval support.
The three models jointly explain \(R^2 =\) 0.88 of the variation in profit,
...
\begin{table}[!h]
\centering
\caption{\label{tab:tab-gam-fit}Approximate significance of smooth and tensor-product terms in the site-profit GAM (Gaussian, REML) and the scenario-level total AFS area GAM (Gamma, log link, REML). Logistics and establishment cost scalers enter the profit GAM as linear parametric terms (see Table~\ref{tab:tab-gam-parametric}); both appear as smooths in the area GAM, with an additional tensor-product interaction surface. $R^2$ and deviance explained: profit GAM $R^2 = 0.879$, dev.~87.9\%; area GAM $R^2 = 0.906$, dev.~92.1\%.}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\fontsize{8}{10}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{5.0cm}>{\raggedleft\arraybackslash}p{1.5cm}>{\raggedleft\arraybackslash}p{1.5cm}>{\raggedleft\arraybackslash}p{1.5cm}>{\raggedright\arraybackslash}p{1.5cm}}
\toprule
Model & Term & Eff.~df & Ref.~df & $F$ & $p$\\
\midrule
\addlinespace[0.3em]
\multicolumn{6}{l}{\textbf{Site-profit GAM — smooth terms}}\\
\hspace{1em}Profit GAM & s(site opp.~cost) & 3.633 & 3.920 & 5646.899 & $<$0.001\\
\hspace{1em}Profit GAM & s(dist.~site--hub, km) & 3.988 & 4.000 & 471.686 & $<$0.001\\
\hspace{1em}Profit GAM & s(avg.~rotation, yr) & 3.997 & 4.000 & 13755.842 & $<$0.001\\
\hspace{1em}Profit GAM & s(log AFS area) & 2.994 & 3.000 & 267.088 & $<$0.001\\
\addlinespace[0.3em]
\multicolumn{6}{l}{\textbf{AFS area GAM — smooth terms}}\\
\hspace{1em}Area GAM & s(opp.~cost scaler) & 1.945 & 1.997 & 535.598 & $<$0.001\\
\hspace{1em}Area GAM & s(logistics cost scaler) & 1.323 & 1.542 & 135.413 & $<$0.001\\
\hspace{1em}Area GAM & s(establishment cost scaler) & 1.987 & 2.000 & 1603.891 & $<$0.001\\
\hspace{1em}Area GAM & s(revenue scaler) & 1.974 & 1.999 & 2089.208 & $<$0.001\\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Partial Effects on Site-Level Profitability}\label{sec:sens-profit}

\autoref{fig:fig-sens-gam-profit} shows the four partial smooth effects
\(\hat{f}_j(\cdot)\) from \texttt{gam\_profit}, visualised using \texttt{gratia::draw()}.
...
{\centering \includegraphics[width=\textwidth]{figures/fig-sens-gam-profit-1} 

}

\caption{GAM partial effects on site-level annual profit $\hat{\pi}^{ha}$ (\euro/ha/yr). Each panel shows the estimated smooth $\hat{f}_j(\cdot)$ with 95\% pointwise confidence bands (grey shading). Dashed vertical line: benchmark value. Rug marks at the bottom indicate the marginal distribution of each predictor across the 810 sensitivity runs.}\label{fig:fig-sens-gam-profit}
\end{figure}
...
age bound \(A^{max} = 20\)\textasciitilde yr to maximise the high-value P1 stem fraction ---
...
\subsection{Partial Effects on area}\label{sec:sens-rotation}

\autoref{fig:fig-sens-gam-rotation} presents the partial smooth effects from
\texttt{gam\_rotation} (left) and \texttt{gam\_p1} (right), revealing how the four experimental
factors shape the optimal rotation strategy and biomass portfolio.

\begin{figure}

{\centering \includegraphics[width=\textwidth]{figures/fig-sens-gam-area-1} 

}

\caption{GAM partial effects on mean rotation length $\bar{a}^{rot}$ (upper row) and stem share $\delta_{P1}$ (lower row). Left: opportunity cost and revenue effects; right: logistics and establishment cost effects. Grey bands: 95\% confidence intervals. Dashed vertical lines: benchmark values.}\label{fig:fig-sens-gam-area}
\end{figure}

The rotation-length model (upper panels) reveals a pronounced negative effect of
opportunity cost on \(\bar{a}^{rot}\): at high \(\bar{c}^{opp}\), the AFS-SCD prefers
...
only approached near stand age 14--18\textasciitilde yr. The logistics and establishment cost
...
\subsection{Implications for Cleaner Production Pathways}\label{sec:sens-implications}
...
\citep{Faasch2012_SRC_Economics, Graves2007_Agroforestry_Economics}. Second, the
...
\texttt{gam\_rotation} shows that chemical-grade biomass supply chains (P1) systematically
promote rotations in the 14--18\textasciitilde yr range, reinforcing the cleaner production
