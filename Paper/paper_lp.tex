...
\begin{align}
  ...
  \sum_{j \in \mathcal{J}} X_{ijpt}    &\leq \sum_{(s,t) \in \mathcal{S}^{{harv}}} \eta^{fw}_{p(t-s)} \cdot z_{ist} && \forall\; i \in \mathcal{I},\; p \in \mathcal{P},\; t \in \mathcal{T}^{{harv}}     \label{eq:C3} \\[2pt]
  %
...
\caption{Site selection and biomass production in optimal supply chain configuration. Green polygons: active sites with biomass harvest; grey polygons: non-selected sites. Pie charts at active sites show the product mix of harvested biomass (stem = P1, branches = P2, residues = P3), scaled by total harvested volume.}\label{fig:fig-results-sc-map}
\end{figure}

In total, on 38 sites AFS are established which corresponds to about 26 \% of all sites and 24 \% of the total farm area. Over the planning horizon, \ensuremath{2.6368\times 10^{4}} kt of AFS biomasses are produced on these sites which suffices to satisfy 57 \% of total demand. \autoref{fig-results-product-mix} shows the harvested biomasses over time along with total demand of all consumer sites.
...
\hspace{1em}Profit GAM & s(site opp.~cost) & 3.750 & 3.955 & 4041.445 & $<$0.001\\
\hspace{1em}Profit GAM & s(dist.~site--hub, km) & 3.962 & 3.999 & 380.365 & $<$0.001\\
\hspace{1em}Profit GAM & s(avg.~rotation, yr) & 3.953 & 3.999 & 247.832 & $<$0.001\\
\hspace{1em}Profit GAM & s(log AFS area) & 2.983 & 3.000 & 117.448 & $<$0.001\\
\addlinespace[0.3em]
\multicolumn{6}{l}{\textbf{AFS area GAM — smooth terms}}\\
\hspace{1em}Area GAM & s(opp.~cost scaler) & 1.906 & 1.991 & 794.190 & $<$0.001\\
\hspace{1em}Area GAM & s(logistics cost scaler) & 1.227 & 1.403 & 396.286 & $<$0.001\\
\hspace{1em}Area GAM & s(establishment cost scaler) & 1.003 & 1.006 & 262.428 & $<$0.001\\
\hspace{1em}Area GAM & s(revenue scaler) & 1.993 & 2.000 & 1783.132 & $<$0.001\\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[!h]
\centering
\caption{\label{tab:tab-gam-parametric}Parametric (linear) coefficients in the site-profit GAM. The three scenario-level cost and revenue scalers are entered as linear terms; a unit increase in each scaler corresponds to the stated change in profit (\euro\,ha$^{-1}$\,yr$^{-1}$). The intercept is the expected profit at scaler $= 0$ and all smooth terms at their mean.}
\centering
\fontsize{9}{11}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{5.2cm}>{\raggedleft\arraybackslash}p{1.8cm}>{\raggedleft\arraybackslash}p{1.8cm}>{\raggedleft\arraybackslash}p{1.8cm}>{\raggedright\arraybackslash}p{1.8cm}}
\toprule
Predictor & $\hat{\beta}$ & SE & $t$ & $p$\\
\midrule
\em{Intercept} & \em{-115.76} & \em{2.68} & \em{-43.20} & \em{$<$0.001}\\
Logistics cost scaler & -86.18 & 0.99 & -86.89 & $<$0.001\\
Establishment cost scaler & -55.89 & 0.96 & -58.00 & $<$0.001\\
Revenue scaler & 544.15 & 2.50 & 217.68 & $<$0.001\\
\bottomrule
\end{tabular}
\end{table}
...
{\centering \includegraphics[width=\textwidth]{figures/fig-gam-profit-smooths-1} 

}

\caption{Partial effects (centred smooths) of the four non-linear terms in the site-profit GAM: site-level opportunity cost, hub distance, average harvest rotation length, and log-transformed AFS strip area. Shaded bands: 95\,\% simultaneous credible intervals. Deviance explained: 73.1\,\%; adjusted $R^2 = 0.731$.}\label{fig:fig-gam-profit-smooths}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=\textwidth]{figures/fig-gam-area-smooths-1} 

}

\caption{Partial effects of scenario-parameter smooths in the scenario-level total AFS area GAM (Gamma family, log link). Panel (e) shows the tensor-product interaction surface of logistics and establishment cost scalers; contours indicate predicted total AFS area on the response scale (ha). Shaded bands in panels (a)--(d): 95\,\% simultaneous credible intervals. Deviance explained: 81.1\,\%; adjusted $R^2 = -0.388$.}\label{fig:fig-gam-area-smooths}
\end{figure}
