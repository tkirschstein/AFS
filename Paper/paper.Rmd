...
consumers <- consumers %>%
  mutate(
    ...
    size = pmin(4.5, pmax(1.5, 1.5 + log1p(total_demand) * 0.55))
  )

storages <- storages %>%
  mutate(
    subtype = if_else(type == "Hub", "Logistics Hub", "Sawmill"),
    size    = pmin(3.5, pmax(1.2, 1.2 + log1p(CAP_proc) * 0.38))
  )

# ── Farben (konsistent mit map_workspace.R / app_v10.r) ──────────────────────
...
COL_P1      <- "#6a0dad"; COL_P2      <- "#08519c"; COL_P3      <- "#a50026"
  ...
  # Zentroide als sf-Punkte
  sites_sf_pts <- st_as_sf(sites, coords = c("lng", "lat"), crs = 4326)
  stor_sf      <- st_as_sf(storages,  coords = c("lng", "lat"), crs = 4326)
  cons_sf      <- st_as_sf(consumers, coords = c("lng", "lat"), crs = 4326)
  ...
  ggplot() +
    ...
    geom_sf(data = stor_sf,
            aes(size = size, shape = subtype),
            fill  = COL_SAW, colour = "white",
            stroke = 0.6, alpha = 0.9,
            show.legend = TRUE) +
    scale_shape_manual(
      name   = "Pre-treatment",
      values = c("Sawmill" = 21, "Logistics Hub" = 24)
    ) +
    # Consumer-Punkte
    ggnewscale::new_scale_fill() +
    geom_sf(data = cons_sf,
            aes(fill = kategorie, size = size),
            shape = 22, colour = "white", stroke = 0.5, alpha = 0.85,
            show.legend = TRUE) +
    scale_fill_manual(
      name   = "Consumer type",
      values = c(
