...
# Modell-Hilfsfunktionen laden
source("../Modell/!helper_func.r")
source("../Modell/!helper_instance_builder_v8a.R")
source("../Modell/build_agroforestry_lp_v10.R")

# Basisdaten laden (Sites, Storages, Consumers, Distanzmatrizen)
#load("../Modell/!base_data.rdata")
...
# Yield-Tabelle aus Chapman-Richards ableiten (3 Produkte)
yields_by_age <- expand_grid(product = 1:3, age = 1:15) %>%
  mutate(
    ...
    # Produktfraktionen: chemisch 20%, Zellstoff 35%, Energie 45%
...
```{r fig-supply-chain-network, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="AFS supply chain network in Saxony-Anhalt. Green polygons: potential KUP sites. Orange circles: pre-treatment facilities (sawmills and logistics hubs, radius proportional to processing capacity). Coloured squares: consumer sites (violet: chemical/pulp, blue: pulp/paper, red: energy/biogas).", fig.width=6.5, fig.height=5.5, out.width="\\textwidth"}
...
# ── Bundeslandgrenzen (rnaturalearth, admin-1) ────────────────────────────────
de_states <- ne_states(country = "Germany", returnclass = "sf") %>%
  st_transform(4326)

# ── Flüsse (rnaturalearth, Skalenstufe medium) ────────────────────────────────
...
storages_plt <- storages %>%
  mutate(
    ...
    col_fill = if_else(type == "Hub", COL_HUB, COL_SAW)
  )
...
ggplot() +
  ...
  # 4. Pre-treatment: colour + shape  (KEIN fill → keine zweite Scale nötig)
  ...
  # 5. neue fill-Scale für Consumer
  ggnewscale::new_scale_fill() +

  # 6. Consumer-Punkte (fill-Quadrate)
  ...
  # 7. Beschriftungen Vorbehandlung
  # ggrepel::geom_label_repel(
  #   data          = stor_sf,
  #   aes(label = name, geometry = geometry),
  #   stat          = "sf_coordinates",
  #   size          = 1.8, label.size = 0.15,
  ...
  # 9. guides(): colour + shape zu einer Legende zusammenführen
  guides(
    colour = guide_legend(
      order        = 1,
    ...
    fill   = guide_legend(
      order        = 2,
      override.aes = list(size = 4, shape = 22, colour = "white")
    ),
    size   = "none"
  ) +
