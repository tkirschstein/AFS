...
:::: {.columns}
::: {.column width="55%"}
![Aboveground biomass components (stems, branches, residues) by stand age for poplar clone Max 3 under SRAFS conditions. Parameterisation: $a^\infty = 100$ t/ha, $\tilde{b}^1 = 0.32$, $\tau = 4.0$ yr.](../Paper/figures/max3-growth-1.png){fig-align="center" width="100%"}
:::
::: {.column width="45%"}
**Key observations:**
...
![Three-echelon supply chain network: AFS sites $\mathcal{I}$ (sources) → storage/processing nodes $\mathcal{J}$ → industrial consumers $\mathcal{K}$ (sinks). Arcs represent raw biomass transport (site → storage) and pre-treated biomass flows (storage → consumer).](../Paper/figures/fig-supply-chain-network-1.pdf){fig-align="center" width="90%"}

## Age-Lag Arc Structure $\mathcal{S}$

:::: {.columns}
::: {.column width="50%"}
![Arc network $\mathcal{S}$: establishment arcs $\mathcal{S}^{est}$, harvest arcs $\mathcal{S}^{harv}$ (feasible only for $A_{min} \leq t-s \leq A_{max}$), and termination arcs $\mathcal{S}^{term}$.](../Paper/figures/fig-set-S-tikz-1.pdf){fig-align="center" width="100%"}
:::
::: {.column width="50%"}
![Arc network $\mathcal{S}$ (alternative view): consecutive harvest arcs enforce path-consistent rotation sequences across the planning horizon $\mathcal{T}$.](../Paper/figures/fig-set-S-tikz2-1.pdf){fig-align="center" width="100%"}
:::
::::
...
:::: {.columns}
::: {.column width="50%"}
![Example path 1: establishment at $t=0$, first harvest at $t=5$ (age 5), second harvest at $t=9$ (age 4), termination at $t=12$. All arcs satisfy $A_{min}=3$, $A_{max}=15$.](../Paper/figures/fig-set-S-path-1.pdf){fig-align="center" width="100%"}
:::
::: {.column width="50%"}
![Example path 2: alternative rotation pattern with earlier first harvest. Path-connectivity constraint C2 ensures no orphaned harvest decisions.](../Paper/figures/fig-set-S-path2-1.pdf){fig-align="center" width="100%"}
:::
::::
...
![Supply chain network for the Saxony-Anhalt case study: AFS site clusters (green), storage/processing nodes (orange), and industrial consumers (blue). Arc widths indicate optimised biomass flow volumes. Key consumers include Mercer Stendal GmbH (pulp) and UPM Biochemicals Leuna (chemicals).](../Paper/figures/fig-results-sc-map-1.pdf){fig-align="center" width="85%"}

## Consumer Demand & Product Classification

:::: {.columns}
::: {.column width="55%"}
![Consumer demand profile by product type (P1 stems, P2 branches, P3 residues) across industrial consumers $k \in \mathcal{K}$. Demand values scaled to 1/5 of raw register values; consumers with total demand < 1.5 units excluded.](../Paper/figures/fig-consumer-demand-1.pdf){fig-align="center" width="100%"}
:::
::: {.column width="45%"}
...
![Optimal supply chain network map: established AFS sites (green dots, size proportional to harvested area), active storage nodes (orange), and served consumers (blue). Arcs show biomass flows aggregated over the planning horizon; arc width proportional to total volume.](../Paper/figures/fig-results-sc-map-1.pdf){fig-align="center" width="88%"}

## Harvest & Rotation Patterns

![Gantt chart of optimal harvest decisions by site cluster and period. Each bar represents an active harvest arc $z_{ist}=1$; bar length equals rotation age $(t-s)$. Age-lag constraints (C2/C3) ensure path-consistent sequences — no orphaned harvest decisions.](../Paper/figures/fig-results-harvest-gantt-1.pdf){fig-align="center" width="95%"}
...
![Alluvial diagram of biomass flows: product type (P1/P2/P3) from harvest sites through storage nodes to industrial consumers. Width of flow bands proportional to annual average volume. Product cascade (C8) visible as P1-stem flows partially satisfying P2/P3 demand.](../Paper/figures/fig-results-product-mix-1.pdf){fig-align="center" width="95%"}

## Inventory Dynamics at Storage Nodes

![Inventory levels $S_{jpt}$ at storage nodes by product type and period. Storage buffers harvest peaks (spring/summer) to ensure continuous supply to consumers. Year-of-harvest drying cost-free; storage costs accrue from period 2 onward.](../Paper/figures/fig-results-inventory-1.pdf){fig-align="center" width="95%"}
