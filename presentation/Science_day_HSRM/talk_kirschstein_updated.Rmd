...
institute: "Fraunhofer IKTS, Dresden, Germany"
date: "`r Sys.Date()`"
output:
...
## Motivation – Biobased materials and woody biomass demand
...
.callout-block[
Total wood production in Germany: ~27 mill. t dry (2024), of which ~4–5 mill. t is beech `r Citep(bib, "StatistischesBundesamt2026_WoodProduction")`.
UPM Biochemicals (Leuna) demands ~10 % of German beech production for biochemical conversion `r Citep(bib, "upmBiochemicals2024")`.
]

---

## Motivation – AFS potential in Germany
...
## AFS Biomass Fractions and Usage Pathways
...
<div style="background:#f4f9f4; border-radius:8px; padding:0.8rem; border-left:4px solid #4a7c2f;">
  ...
  <div style="margin:0.4rem 0; border-radius:4px; overflow:hidden; font-size:0.9rem; background:#6ea048; color:white;">
    <div style="padding:0.3rem 0.5rem;">🍃 <strong>Residues</strong> (Ø < 7 cm + leaves)</div>
  </div>
</div>

<div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:0.6rem;">
  <div style="background:#fff; border:1px solid #c8dfc0; border-radius:8px; padding:0.6rem; text-align:center;">
    <p style="font-weight:700; font-size:0.95rem; color:#2d5a1b;">Chemical industry</p>
    <p style="font-size:0.65rem; color:#555; line-height:1.3;">Bioplastics, lignin derivatives. Gate price: 80–120 €/t</p>
    <div style="padding:0.15rem; background:#e8f4e0; border-radius:3px; font-size:0.6rem; color:#4a7c2f;">▶ Stem (P1)</div>
  </div>
  <div style="background:#fff; border:1px solid #c8dfc0; border-radius:8px; padding:0.6rem; text-align:center;">
    <p style="font-weight:700; font-size:0.95rem; color:#2d5a1b;">Pulp & Paper</p>
    <p style="font-size:0.65rem; color:#555; line-height:1.3;">Long-fibre hardwood pulp, cardboard. Gate price: 55–75 €/t</p>
    <div style="padding:0.15rem; background:#e8f4e0; border-radius:3px; font-size:0.6rem; color:#4a7c2f;">▶ Stem + Branches (P2)</div>
  </div>
  <div style="background:#fff; border:1px solid #c8dfc0; border-radius:8px; padding:0.6rem; text-align:center;">
    <p style="font-weight:700; font-size:0.95rem; color:#2d5a1b;">Energy generation</p>
    <p style="font-size:0.65rem; color:#555; line-height:1.3;">Wood chips, pellets, CHP. Gate price: 30–40 €/t</p>
    <div style="padding:0.15rem; background:#fff3e0; border-radius:3px; font-size:0.6rem; color:#b45309;">▶ All fractions (P3)</div>
  </div>
</div>
</div>

.footnote[Cascade utilisation following EU Bioeconomy Strategy (2018); price ranges: `r Citep(bib, "Lamers2015_Feedstock_Design")`]
...
- No inter-row crop production `r Citep(bib, "Trnka2008_Poplar_SRF")`

**Short-Rotation AFS (SRAFS)**
- 1–4 tree rows in alley-cropping
- Tree strips: 10–30 % of total parcel area
- Edge-tree effect → higher individual-tree biomass `r Citep(bib, "Huber2018_SRAFS_growth")`

**Long-Rotation AFS (LRAFS)**
- Rotation ≥ 12–15 years
- Large stem diameters → high-value timber `r Citep(bib, "Graves2007_Agroforestry_Economics")`
]

.col-60[
.callout-block[
In this paper **SRAFS and LRAFS are not treated as fixed categories**. The MILP optimises rotation length endogenously, spanning the full range [A_min, A_max].
]
...
- $A(N) = C_{\text{site}} \cdot N^{1-\beta}$ — density-dependent asymptote
- $k \in [0.17, 0.2]$ yr⁻¹, $t_0 \in [9, 10]$ yr for poplar in Central Europe
- $t_0$ indicates the upper bound on economically optimal rotation age

**Compartment fractions** (logistic model):
$$q_p(t) = \frac{f_p}{1 + \exp(-r_p \cdot (t - t_{50,p}))}$$

Calibrated for stem ($d > 15$ cm): $r_s = 0.467$, $t_{50,s} = 8.19$ yr, $f_s = 0.75$  
Branch ($d > 7$ cm): $r_b = 0.698$, $t_{50,b} = 7.55$ yr, $f_b = 0.15$  
`r Citep(bib, "Jha2018_Poplar_AFS_Biomass")`; `r Citep(bib, "Civitarese2019_Poplar_Harvest")`
]
]

.col-60[
```{r max3-growth, fig.height=4.5, fig.width=6, out.width="95%", cache=TRUE}
...
ggplot(comp_long, aes(x = age, y = biomass)) +
  ...
  labs(x = "Stand age (years)", y = expression("biomass (t DM "*ha^{-1}*")"),
...
- **Layer 1:** AFS sites $\mathcal{I}$ — potential poplar alley-cropping parcels
- **Layer 2:** Pre-treatment / storage nodes $\mathcal{J}$ — sawmills and logistics hubs
- **Layer 3:** Industrial consumers $\mathcal{K}$ — chemical, pulp/paper, energy sectors
...
```{r tikz-netzwerk-1, engine='tikz', fig.ext='png', fig.align='center', out.width='80%', echo=FALSE,
  engine.opts=list(extra.preamble=c("\\usepackage{tikz}", "\\usetikzlibrary{arrows.meta,shapes.geometric,shapes.misc}")),
...
```{r tikz-netzwerk-2, engine='tikz', fig.ext='png', fig.align='center', out.width='80%', echo=FALSE,
  engine.opts=list(extra.preamble=c("\\usepackage{tikz}", "\\usetikzlibrary{arrows.meta,shapes.geometric,shapes.misc}")),
  fig.height = 3, cache=TRUE}
...
site_harvest <- ext$Xij %>%
  ...
  tidyr::pivot_wider(names_from = product, values_from = vol, values_fill = 0, names_prefix = "p")
