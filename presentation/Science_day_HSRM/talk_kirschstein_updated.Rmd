...
```{r xaringan-leaflet-fix, echo=FALSE, results='asis'}
cat("
<script>
function resizeVisibleLeafletMaps() {
  document.querySelectorAll('.remark-visible .leaflet, .remark-visible .leaflet-container, .remark-visible .html-widget').forEach(function(el){
    try {
      if (el._leaflet_map) {
...
document.addEventListener('DOMContentLoaded', function () {
  setTimeout(resizeVisibleLeafletMaps, 400);
  setTimeout(resizeVisibleLeafletMaps, 1200);
});
document.addEventListener('slideenter', function () {
  setTimeout(resizeVisibleLeafletMaps, 100);
  setTimeout(resizeVisibleLeafletMaps, 500);
  setTimeout(resizeVisibleLeafletMaps, 1200);
});
</script>
")
...
leaflet(
  ...
  function(el, x) {
    var map = this;
    function fix() { map.invalidateSize({animate: false, pan: false}); }
    setTimeout(fix, 2000); setTimeout(fix, 8000); setTimeout(fix, 15000);
    // Remark.js slideenter
    document.addEventListener('slideenter', function() { setTimeout(fix, 1500); });
  }
")
```
...
```{r fig-supply-chain-network-leaflet, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%", cache=TRUE}
...
map_sc <- leaflet(
  options = leafletOptions(zoomControl = TRUE, preferCanvas = TRUE),
  width = "100%",
  height = 430
) %>%
  addProviderTiles(providers$CartoDB.Positron, group = "Basemap") %>%
  addPolygons(
    data = de_states,
    color = "grey55", weight = 0.8,
    fillColor = "grey96", fillOpacity = 0.7,
    smoothFactor = 0.2, group = "States"
  ) %>%
  addPolylines(
    data = rivers_eu,
  ...
  addPolygons(
    data = sites_sf_clust,
    stroke = TRUE, color = "grey30", weight = 0.3,
    fillColor = ~pal_opp(C_opp), fillOpacity = 0.65,
    popup = ~paste0(
  ...
  addLayersControl(
    overlayGroups = c("States", "Rivers", "AFS sites", "Hubs", "Consumers"),
  ...
  function(el, x) {
    var map = this;
    function fix() { map.invalidateSize({animate: false, pan: false}); }
    setTimeout(fix, 200); setTimeout(fix, 800); setTimeout(fix, 1500);
    // Remark.js slideenter
    document.addEventListener('slideenter', function() { setTimeout(fix, 150); });
  }
")
