...
consumers_plt <- consumers %>%
  mutate(
    ...
    pt_size = pmin(14, pmax(5, 5 + log1p(total_demand) * 1.8))
  )

storages_plt <- storages %>%
  mutate(subtype = "Hub", pt_size = 8)
...
leaflet(options = leafletOptions(zoomControl = TRUE)) %>%
  ...
  addCircleMarkers(
    ...
    color = "white",
    weight = 1,
    fillColor = ~pal_cons(kategorie),
    fillOpacity = 0.9,
    popup = ~paste0(
      "<strong>Consumer type:</strong> ", kategorie,
      "<br><strong>Total demand:</strong> ", round(total_demand, 1)
    ),
    group = "Consumers"
  ) %>%
  addLegend(
