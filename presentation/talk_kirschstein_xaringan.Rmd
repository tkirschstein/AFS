...
- Parameters calibrated from SRAFS field trials in Southern Germany
...
.pull-left[
```{r gompertz-fig, out.width="100%"}
knitr::include_graphics("fig/max3-growth-1.pdf")
```
_Aboveground biomass components (stems, branches, residues) by stand age for poplar clone Max 3 under SRAFS conditions. Parameterisation: $a^\infty = 100$ t/ha, $\tilde{b}^1 = 0.32$, $\tau = 4.0$ yr._
]
...
```{r sc-network, out.width="85%", fig.cap="Three-echelon supply chain network: AFS sites (sources) → storage/processing nodes → industrial consumers (sinks)."}
knitr::include_graphics("fig/fig-supply-chain-network-1.pdf")
```
...
.pull-left[
```{r arc-tikz1, out.width="100%", fig.cap="Arc network S: establishment arcs, harvest arcs (feasible only for A_min ≤ t−s ≤ A_max), and termination arcs."}
knitr::include_graphics("fig/fig-set-S-tikz-1.pdf")
```
]

.pull-right[
```{r arc-tikz2, out.width="100%", fig.cap="Arc network S (alternative view): consecutive harvest arcs enforce path-consistent rotation sequences."}
knitr::include_graphics("fig/fig-set-S-tikz2-1.pdf")
```
]
...
.pull-left[
```{r path1, out.width="100%", fig.cap="Example path 1: establishment at t=0, first harvest at t=5 (age 5), second harvest at t=9 (age 4), termination at t=12."}
knitr::include_graphics("fig/fig-set-S-path-1.pdf")
```
]

.pull-right[
```{r path2, out.width="100%", fig.cap="Example path 2: alternative rotation pattern with earlier first harvest."}
knitr::include_graphics("fig/fig-set-S-path2-1.pdf")
```
]
...
**(C4)/(C5) Transport linkage & inventory balance** — follows OPTIMASS network-flow structure
...
```{r clustering, out.width="88%", fig.cap="Clustering comparison: INVEKOS SRC field parcels (left) clustered into representative site groups via DBSCAN + Ward.D2 HAC (right). Cluster centroids define site locations."}
knitr::include_graphics("../Paper/plot_clustering_comparison.png")
```
...
```{r sc-map, out.width="85%", fig.cap="Supply chain network for the Saxony-Anhalt case study: AFS site clusters (green), storage/processing nodes (orange), and industrial consumers (blue)."}
knitr::include_graphics("fig/fig-results-sc-map-1.pdf")
```
...
.pull-left[
```{r consumer-demand, out.width="100%", fig.cap="Consumer demand profile by product type (P1 stems, P2 branches, P3 residues) across industrial consumers."}
knitr::include_graphics("fig/fig-consumer-demand-1.pdf")
```
]
...
| P3 — Residues | Low quality | Energy (combustion, biogas) | End of cascade |
]
...
```{r sc-map-opt, out.width="88%", fig.cap="Optimal supply chain network map: established AFS sites (green dots, size proportional to harvested area), active storage nodes (orange), and served consumers (blue)."}
knitr::include_graphics("fig/fig-results-sc-map-1.pdf")
```
...
```{r harvest-gantt, out.width="95%", fig.cap="Gantt chart of optimal harvest decisions by site cluster and period. Bar length equals rotation age (t−s). Age-lag constraints ensure path-consistent sequences."}
knitr::include_graphics("fig/fig-results-harvest-gantt-1.pdf")
```

.blockquote[
**Trade-off:** Longer rotations → higher stem share & revenue per harvest | Shorter rotations → more frequent harvests, better demand coverage
]
...
```{r product-mix, out.width="95%", fig.cap="Alluvial diagram of biomass flows: product type (P1/P2/P3) from harvest sites through storage nodes to industrial consumers. Product cascade (C8) visible as P1-stem flows partially satisfying P2/P3 demand."}
knitr::include_graphics("fig/fig-results-product-mix-1.pdf")
```
...
```{r inventory, out.width="95%", fig.cap="Inventory levels at storage nodes by product type and period. Storage buffers harvest peaks to ensure continuous supply to consumers."}
knitr::include_graphics("fig/fig-results-inventory-1.pdf")
```
...
# Mit RefManageR oder manuelle Referenzen
# PrintBibliography(bib)
```

_Full bibliography: see manuscript references._
