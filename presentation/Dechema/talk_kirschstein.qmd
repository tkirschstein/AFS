...
subtitle: "Benefits of Agroforestry Systems for Defossilizing Chemical Industry"
author: "Thomas Kirschstein and Nico Tauchnitz"
institute: "Fraunhofer IKTS"
date: "Frankfurt, am 16. September 2026"
...
format: 
  revealjs:
    ...
    scrollable: false          # Scrollbars auf Folien verhindern
    #fig-responsive: false      # Plotly-Responsive-Mechanismus deaktivieren
...
source("../../paper/!helper_extract_site_profit_paper.R")
...
price.range.tab <- milp_instance$consumer_prices |> 
  mutate(price = ifelse(price == 0, NA, price)) |> 
  group_by(product) |> 
  summarise(min = min(price, na.rm=T), max = max(price, na.rm=T), .groups = "drop")
...
\max\; \text{Total profit} \;=\;&
  \text{Revenue}  \\
&- \text{Establishment cost}\\
&- \text{Maintenance}\\
& - \text{Opportunity cost}\\
&- \text{Harvest cost}\\
&- \text{Raw wood transport cost}\\
&- \text{Pre-processed wood transport cost}\\
&- \text{Storage cost}
\end{aligned}$$
:::
...
|--------------|-----------------------|------------|
| 1 Chemical | Mercer Stendal, UPM Leuna | `r round(price.range.tab$min[price.range.tab$product == "1"])`-`r round(price.range.tab$max[price.range.tab$product == "1"])` €/t |
| 2 Pulp     | saw mills, pellet producer   | `r round(price.range.tab$min[price.range.tab$product == "2"])`-`r round(price.range.tab$max[price.range.tab$product == "2"])` €/t  |
| 3 Energy   | bio mass fermenters, composting facility | `r round(price.range.tab$min[price.range.tab$product == "3"])`-`r round(price.range.tab$max[price.range.tab$product == "3"])` €/t  |
:::
:::
::::::
...
map_sc <- map_sc <- leaflet(
  ...
  addPolygons(
    data = sites_sf_clust,
    stroke = TRUE, color = "grey30", weight = 1.5,
    fillColor = ~pal_opp(C_opp), fillOpacity = 1,
    popup = ~paste0(
  ...
  addCircleMarkers(
    data = storsf,
    radius = ~ptsize*.85,
    color = COLHUB, stroke = TRUE, weight = 1,
    fillColor = COLHUB, fillOpacity = 0.85,
...
| Planning horizon $T$ | `r milp_instance$n_periods`  years |
| Max rotation age $A_{\max}$ | `r milp_instance$max_age` years |
| Min rotation age $A_{\min}$ | `r milp_instance$min_age` years |
| Raw transport $c^{tr\text{-}raw}$ | `r milp_instance$c_tr_raw` €/(t·km) |
| Pre-treated transport $c^{tr\text{-}pre}$ | `r milp_instance$c_tr_pre` €/(t·km) |
| Establishment cost $c^{est}$ | `r milp_instance$sites$C_est[1]` €/ha |
| Harvest cost $c^{harv}$ | `r milp_instance$sites$C_harv[1]` €/ha |
...
```{r , echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=4.25, out.width="99%", cache=FALSE}
...
pp <- ggplot(flow_df, aes(axis1 = src, axis2 = del, y = volume/1000, fill = src)) +
  ...
  scale_y_continuous(name = "wood quantity (kt)",
...
shipped_ts <- ext$Xij %>%
  group_by(period) %>%
  summarise(shipped = sum(value)/1000, .groups = "drop")

pp <- ggplot(delivery_ts, aes(x = period, y = volume, fill = product)) +
  ...
  scale_y_continuous(name = "Volume (kt)", labels = scales::comma) +
...
# prod_order <- c("1", "2", "3")
# 
# # ── Zeitreihen: Lieferungen Hub → Kunde nach ausgeliefertem Produkt ───────────
# delivery_ts <- ext$Xjk %>%
#   mutate(product = as.character(del_product)) %>%
#   group_by(period, product) %>%
#   summarise(volume = sum(value, na.rm = TRUE) / 1000, .groups = "drop") %>%
#   tidyr::complete(
#     period  = seq_len(milp_instance$n_periods),
#     product = prod_order,
#     fill    = list(volume = 0)
#   ) %>%
#   mutate(
#     product = factor(product, levels = prod_order),
#     label   = unname(prod_labels[as.character(product)])
#   ) %>%
#   arrange(product, period)
# 
# # Rohholzmenge: Standort → Hub
# shipped_ts <- ext$Xij %>%
#   group_by(period) %>%
#   summarise(shipped = sum(value, na.rm = TRUE) / 1000, .groups = "drop") %>%
#   tidyr::complete(
#     period = seq_len(milp_instance$n_periods),
#     fill = list(shipped = 0)
#   )
# 
# # ── Plotly: gestapelte Flächen ─────────────────────────────────────────────────
# fig_biomass <- plot_ly()
# 
# for (p in prod_order) {
# 
#   df_p <- delivery_ts %>%
#     filter(as.character(product) == p)
# 
#   fig_biomass <- fig_biomass %>%
#     add_trace(
#       data      = df_p,
#       x         = ~period,
#       y         = ~volume,
#       type      = "scatter",
#       mode      = "lines",
#       stackgroup = "deliveries",
#       name      = prod_labels[[p]],
#       legendgroup = p,
#       fillcolor = scales::alpha(prod_cols[[p]], 0.80),
#       line      = list(
#         color = "white",
#         width = 0.5
#       ),
#       text = ~paste0(
#         "<b>", prod_labels[[p]], "</b><br>",
#         "Planning period: ", period, "<br>",
#         "Delivered volume: ", scales::comma(round(volume, 2)), " kt"
#       ),
#       hoverinfo = "text"
#     )
# }
# 
# # Gestrichelte Referenzlinie: Rohholztransport Standort → Hub
# fig_biomass <- fig_biomass %>%
#   add_trace(
#     data       = shipped_ts,
#     x          = ~period,
...
#     type       = "scatter",
#     mode       = "lines",
#     name       = "Raw shipped (site → hub)",
#     legendgroup = "raw_shipped",
#     line       = list(
#       color = "#636363",
#       width = 1.5,
#       dash  = "dash"
#     ),
#     text = ~paste0(
#       "<b>Raw shipped (site → hub)</b><br>",
#       "Planning period: ", period, "<br>",
#       "Raw wood volume: ", scales::comma(round(shipped, 2)), " kt"
#     ),
#     hoverinfo = "text"
#   )
# 
# # ── Annotation an der Endperiode ───────────────────────────────────────────────
# last_period  <- max(shipped_ts$period, na.rm = TRUE)
# last_shipped <- shipped_ts %>%
...
#       tickmode   = "array",
#       tickvals   = seq(1, milp_instance$n_periods, by = 2),
#       zeroline   = FALSE,
#       showgrid   = TRUE,
#       gridcolor  = "#e5e5e5"
#     ),
...
#         x          = last_period - 1,
#         y          = last_shipped * 0.95,
#         text       = "Raw shipped<br>(site → hub)",
#         showarrow  = FALSE,
#         xanchor    = "right",
#         yanchor    = "middle",
#         font       = list(size = 11, color = "#636363"),
#         align      = "right"
#       )
#     ),
#     legend = list(
#       title       = list(text = "Product"),
#       orientation = "h",
#       x           = .5,
#       y           = 1,
#       xanchor     = "left",
...
#     paper_bgcolor = "white",
#     plot_bgcolor  = "white",
#     margin        = list(l = 60, r = 150, t = 20, b = 60),
#     hovermode     = "x unified",
#     autosize      = TRUE
#   )
# 
...
subplot(p_wf, p_cf, nrows = 1, widths = c(0.37, 0.55),
  ...
  layout(
    ...
    margin = list(b = 100),
    autosize = T,
    width    = 1000,      # NULL = von CSS gesteuert
    height   = 480,       # px – passend für Reveal.js-Folie mit Titel
...
fig %>%
  layout(
    ...
    plot_bgcolor  = "#f9f9f9",
    autosize = T,
    width    = 900,      # NULL = von CSS gesteuert
    height   = 480,       # px – passend für Reveal.js-Folie mit Titel
    margin   = list(l = 40, r = 20, t = 30, b = 40)
  )
...
## Supply Cost vs. Transport Distance

```{r}
#| label: production-costs-per-site
#| echo: false
#| message: false
#| warning: false
#| cache: true
...
# ══════════════════════════════════════════════════════════════════════════════
# Produktionskosten je AFS-Standort
# Methodik: Market-Value-Allokation der standortbezogenen Kosten auf Fraktionen
#
# Kostenkomponenten (alle €/ha):
#   C_est   – Etablierungskosten (einmalig bei Establishment-Arc)
#   C_harv  – Erntekosten        (je Ernte-Arc)
#   C_main  – Pflegekosten       (je Vegetationsjahr im Ernte-Arc)
#   C_opp   – Opportunitätskosten (je Vegetationsjahr im Ernte-Arc)
#   c_tr_raw– Rohholztransport   Site → Hub (€/(t·km))
#
# Normierung: €/ha/a über aktive Produktionsjahre je Standort
# ══════════════════════════════════════════════════════════════════════════════

# ── 0. Marktpreisgewichte für Fraktionsallokation ─────────────────────────────
# w_p = (median_price_p * mean_yield_share_p) / sum(...)
# Ertragsanteile aus yields_by_age (über alle Altersklassen gemittelt)
yield_shares <- yields_by_age %>%
  group_by(product) %>%
  summarise(mean_yield = mean(yield_ha, na.rm = TRUE), .groups = "drop") %>%
  mutate(total_yield = sum(mean_yield),
         yield_share = mean_yield / total_yield)
...
median_prices <- price_df %>%
  group_by(del_product) %>%
  summarise(med_price = median(price, na.rm = TRUE), .groups = "drop") %>%
  rename(product = del_product)

alloc_weights <- yield_shares %>%
  left_join(median_prices, by = "product") %>%
  mutate(
    mv_raw = yield_share * med_price,   # Marktwertscore je Fraktion
    w_p    = mv_raw / sum(mv_raw)       # normierte Gewichte (sum = 1)
  )

# ── 1. Normierungsbasis: aktive Fläche × aktive Jahre je Standort ─────────────
active_site_ids <- ext$z %>%
  filter(arc_type == "harvest") %>%
  pull(site_id) %>% unique()
...
est_p  <- ext$z %>%
  filter(arc_type == "establishment") %>%
  group_by(site_id) %>%
  summarise(t_est    = sum(t * value) / sum(value),
            area_ha  = sum(value),       # ha etabliert
            .groups  = "drop")
...
site_norm <- site_meta %>%
  left_join(est_p |> select(-area_ha),  by = "site_id") %>%
  left_join(term_p, by = "site_id") %>%
  mutate(
    t_term       = coalesce(t_term, as.numeric(milp_instance$n_periods)),
    active_years = pmax(t_term - t_est, 1),
    denom        = area_ha * active_years   # ha·a (Normierungsnenner)
  )

# ── 2. Kostenkomponenten je Standort (absolut in €) ───────────────────────────
dist_ij_m <- as.matrix(milp_instance$d_ij)

# 2a Etablierung (einmalig)
sc_est <- ext$z %>%
  filter(arc_type == "establishment") %>%
  left_join(site_meta %>% select(site_id, C_est), by = "site_id") %>%
  mutate(cost = C_est * value) %>%
  group_by(site_id) %>%
  summarise(C_establishment = sum(cost, na.rm = TRUE), .groups = "drop")

# 2b Ernte
sc_harv <- ext$z %>%
  filter(arc_type == "harvest") %>%
  left_join(site_meta %>% select(site_id, C_harv), by = "site_id") %>%
  mutate(cost = C_harv * value) %>%
  group_by(site_id) %>%
  summarise(C_harvest = sum(cost, na.rm = TRUE), .groups = "drop")

# 2c Pflege (proportional zu Rotationslänge × Fläche)
sc_main <- ext$z %>%
  filter(arc_type == "harvest") %>%
  mutate(age_len = t - s) %>%
  left_join(site_meta %>% select(site_id, C_main), by = "site_id") %>%
  mutate(cost = C_main * value * age_len) %>%
  group_by(site_id) %>%
  summarise(C_maintenance = sum(cost, na.rm = TRUE), .groups = "drop")

# 2d Opportunitätskosten (standortspezifisch aus sites_clustered$C_opp)
sc_opp <- ext$z %>%
  filter(arc_type == "harvest") %>%
  mutate(age_len = t - s) %>%
  left_join(site_meta %>% select(site_id, C_opp), by = "site_id") %>%
  mutate(cost = C_opp * value * age_len) %>%
  group_by(site_id) %>%
  summarise(C_opportunity = sum(cost, na.rm = TRUE), .groups = "drop")

# 2e Rohholztransport Site → Hub
sc_tr_raw <- ext$Xij %>%
  mutate(
    dist_km = mapply(function(i, j) dist_ij_m[i, j], site_id, hub_id),
  ...
  group_by(site_id) %>%
  summarise(C_transport_raw = sum(cost, na.rm = TRUE), .groups = "drop")

# ── 3. Zusammenführung: Gesamtproduktionskosten je Standort ──────────────────
prod_costs <- site_norm %>%
  select(site_id, area_ha, active_years, denom, C_opp) %>%
  left_join(sc_est,     by = "site_id") %>%
  left_join(sc_harv,    by = "site_id") %>%
  left_join(sc_main,    by = "site_id") %>%
  left_join(sc_opp,     by = "site_id") %>%
  left_join(sc_tr_raw,  by = "site_id") %>%
  replace_na(list(C_establishment = 0, C_harvest = 0,
                  C_maintenance   = 0, C_opportunity = 0,
                  C_transport_raw = 0)) %>%
  mutate(
    C_total       = C_establishment + C_harvest + C_maintenance +
                    C_opportunity   + C_transport_raw,
    # Normierung auf €/ha/a
    across(starts_with("C_"), ~ .x / denom, .names = "{.col}_ha_yr")
  )

# ── 4. Market-Value-Allokation: Produktionskosten → Fraktionen ───────────────
# C_prod_p_i = C_total_i * w_p  (€ absolut)
# c_prod_p_i = C_prod_p_i / Q_p_i  (€/t, falls Mengen bekannt)

# Gelieferte Mengen je Standort und Fraktion (Rohholz via Xij, Produkt p)
vol_site_frac <- ext$Xij %>%
  group_by(site_id, p) %>%
  summarise(vol_t = sum(value, na.rm = TRUE), .groups = "drop") %>%
  rename(product = p)

prod_costs_frac <- prod_costs %>%
  select(site_id, C_total, C_total_ha_yr, C_opp) %>%
  crossing(alloc_weights %>% select(product, w_p)) %>%
  mutate(C_prod_p_abs = C_total * w_p) %>%          # €, Anteil je Fraktion
  left_join(vol_site_frac, by = c("site_id", "product")) %>%
  mutate(
    vol_t     = coalesce(vol_t, 0),
    c_prod_p  = if_else(vol_t > 0, C_prod_p_abs / vol_t, NA_real_), # €/t
    fraction  = factor(
      case_when(product == 1 ~ "Stem (P1)",
                product == 2 ~ "Branches (P2)",
                TRUE         ~ "Residues (P3)"),
      levels = c("Stem (P1)", "Branches (P2)", "Residues (P3)")
    )
  )

# ── 5. Zusammenfassung: Kostentabelle je Standort (long format) ───────────────
prod_costs_long <- prod_costs %>%
  select(site_id, area_ha, C_opp, ends_with("_ha_yr")) %>%
  pivot_longer(
    cols      = ends_with("_ha_yr"),
    names_to  = "component",
    values_to = "cost_ha_yr"
  ) %>%
  mutate(component = str_remove(component, "_ha_yr") %>%
           str_replace_all("_", " ") %>%
           str_to_title() %>%
           str_replace("C ", ""))
...
# ══════════════════════════════════════════════════════════════════════════════
# Stückkosten (€/t) je Holzprodukt × AFS-Standort
#
# Methodik:
#   1. Gesamtproduktionskosten C_total_i (€) aus prod_costs
#   2. Fraktionsallokation via Market-Value-Gewichte w_p (aus alloc_weights)
#   3. Division durch gelieferte Menge Q_p,i → c_unit_p,i in €/t
#
# Zusätzlich:
#   - Transportkosten Hub→Kunde (c_tr_pre × d_jk) werden separat ausgewiesen
#   - Gesamte Stückkosten = c_prod_p,i + c_tr_p,i (Supply Cost, SC_kp)
# ══════════════════════════════════════════════════════════════════════════════

dist_jk_m <- as.matrix(milp_instance$d_jk)

# ── 1. Liefermengen je Standort × Fraktion (Rohholz, t Frischgewicht) ─────────
vol_site_frac <- ext$Xij %>%
  group_by(site_id, p) %>%
   mutate(
    dist_km = mapply(function(i, j) dist_ij_m[i, j], site_id, hub_id),
  ...
  summarise(
    avg_dist_site_km = mean(dist_km, na.rm = TRUE),
    vol_raw_t = sum(value, na.rm = TRUE),
    .groups = "drop") %>%
  ungroup() %>%
  group_by(site_id) %>%
  mutate(vol_total = sum(vol_raw_t)) |> 
  ungroup() %>%
  group_by(site_id, p) %>%
  mutate(share_vol = vol_raw_t/vol_total) |> 
  rename(product = p)

# ── 2. Hub-Anteile je Standort × Periode (für Transportkostenzurechnung) ─────
hub_site_share <- ext$Xij %>%
  group_by(hub_id, period) %>%
  mutate(hub_total = sum(value)) %>%
  ungroup() %>%
  mutate(share = if_else(hub_total > 0, value / hub_total, 0)) %>%
  select(site_id, hub_id ,  period, share)

# ── 3. Transportkosten Hub→Kunde anteilig auf Standort × Fraktion ─────────────
# Xjk enthält: hub_id, consumer_id, del_product, period, value (t geliefert)
tr_pre_site_frac <- ext$Xjk %>%
  mutate(
    dist_km  = mapply(function(j, k) dist_jk_m[j, k], hub_id, consumer_id),
  ...
  summarise(
    C_tr_pre     = sum(cost_jkt * share, na.rm = TRUE),   # €
    vol_del_t    = sum(value    * share, na.rm = TRUE),   # t (zugerechnet)
    avg_dist_km  = weighted.mean(dist_km,
                                 w = pmax(value * share, 0),
                                 na.rm = TRUE),
    .groups = "drop"
  ) %>%
  rename(product = del_product)

# ── 4. Produktionskosten-Allokation (Market-Value-Gewichte) ───────────────────
# alloc_weights$w_p aus Chunk "production-costs-per-site"
unit_costs <- prod_costs %>%
  select(site_id, C_total, area_ha, C_opp, active_years) %>%
  # Liefermengen anhängen
  left_join(vol_site_frac, by = c("site_id")) %>%
  mutate(C_prod_p = C_total * share_vol) %>%             # € Produktionskosten, Anteil p
  
  # Transportkosten anhängen
  left_join(tr_pre_site_frac, by = c("site_id", "product")) %>%
  mutate(
    vol_raw_t    = coalesce(vol_raw_t, 0),
    vol_del_t    = coalesce(vol_del_t, 0),
    C_tr_pre     = coalesce(C_tr_pre, 0),
    avg_dist_km  = coalesce(avg_dist_km, NA_real_),
    avg_dist_site_km  = coalesce(avg_dist_site_km, NA_real_),
    avg_dist_km_total = avg_dist_site_km+avg_dist_km,

    # ── Stückkosten (€/t) ────────────────────────────────────────────────────
    # c_prod_p,i  = allokierte Produktionskosten / gelieferte Menge
    c_prod       = if_else(vol_raw_t > 0, C_prod_p / vol_raw_t, NA_real_),
    # c_tr_p,i    = Transportkosten Hub→Kunde / zugerechnete Menge
    c_tr         = if_else(vol_del_t > 0, C_tr_pre  / vol_del_t, NA_real_),
    # SC_p,i      = Gesamte Stückkosten (Supply Cost)
    c_unit_total = c_prod + coalesce(c_tr, 0),

    # Faktor-Variable Fraktion
    fraction = factor(
      case_when(product == 1 ~ "Stem (P1)",
                product == 2 ~ "Branches (P2)",
                TRUE         ~ "Residues (P3)"),
  ...
  filter(!is.na(c_prod))   # nur Standorte mit tatsächlicher Lieferung
...
#| message: false
#| warning: false
#| dependson: "unit-costs-per-product-site"
...
# Plotly Scatter: c_unit_total (€/t) vs. C_opp (€/ha/a), je Standort × Fraktion
fig_uc <- plot_ly()

for (frac in levels(unit_costs$fraction)) {
  df_f  <- unit_costs %>% filter(fraction == frac, !is.na(c_unit_total))
  col_f <- PAL_FRAC[[frac]]
  #price_f <- first(df_f$med_price)
  ...
  fig_uc <- fig_uc %>%
    add_trace(
      ...
      marker    = list(
        color    = col_f,
        size     = ~pmax(.6, sqrt(vol_raw_t) * 0.022),
        sizemode = "diameter",
        opacity  = 0.75,
        line     = list(color = "white", width = 0.5)
      ),
      text      = ~paste0(
        "<b>Site ", site_id, " · ", frac, "</b><br>",
        "Opp. cost:   ", round(C_opp,        0), " €/ha/a<br>",
        "c_prod:      ", round(c_prod,        1), " €/t<br>",
        "c_transport: ", round(coalesce(c_tr, 0), 1), " €/t<br>",
        "<b>SC total: ", round(c_unit_total,  1), " €/t</b><br>",
        #"Market price:", round(med_price,     1), " €/t<br>",
        #"Margin:      ", round(margin,        1), " €/t<br>",
        "Volume:      ", round(vol_raw_t,     0), " t"
      ),
      hoverinfo  = "text",
      showlegend = TRUE
    )

  # Volumengewichtete Regressionslinie
  if (nrow(df_f) >= 3) {
    lm_f  <- lm(c_unit_total ~ avg_dist_km_total, data = df_f, weights = df_f$vol_raw_t)
    x_seq <- seq(min(df_f$avg_dist_km_total, na.rm = TRUE),
                 max(df_f$avg_dist_km_total, na.rm = TRUE),
                 length.out = 80)
    y_hat <- predict(lm_f, newdata = data.frame(avg_dist_km_total = x_seq))

    fig_uc <- fig_uc %>%
      add_lines(
        x         = x_seq,
        y         = y_hat,
        name      = paste0(frac, " (Trend)"),
        line      = list(color = col_f, width = 1.8, dash = "dot"),
        showlegend = FALSE,
        hoverinfo  = "skip"
      )
  }
...
# ── Horizontale Preisreferenzlinien ───────────────────────────────────────────

x_seq <- round(c(min(unit_costs$avg_dist_km_total, na.rm = TRUE), max(unit_costs$avg_dist_km_total, na.rm = TRUE)))

fig_uc <- fig_uc |> 
  add_trace(x = x_seq, y = rep(price.range.tab$min[1],2), type = "scatter", mode = "lines" ,
            line     = list(color = PAL_FRAC[1], width = 0.5 ), showlegend=F) |> 
  add_trace(x = x_seq, y = rep(price.range.tab$max[1],2), type = "scatter", mode = "lines", fill = 'tonexty', fillcolor = "#1b9e7720",  
            line = list(color = PAL_FRAC[1], width = 0.5), showlegend=T, name="Price range Stems (P1)")
  
fig_uc <- fig_uc |> 
  add_trace(x = x_seq, y = rep(price.range.tab$min[2],2), type = "scatter", mode = "lines",
            line     = list(color = PAL_FRAC[2], width = 0.5 ), showlegend=F) |> 
  add_trace(x = x_seq, y = rep(price.range.tab$max[2],2), type = "scatter", mode = "lines", fill = 'tonexty', fillcolor = "#d95f0220",  
            line = list(color = PAL_FRAC[2], width = 0.5), showlegend=T, name="Price range Branches (P2)")

fig_uc <- fig_uc |> 
  add_trace(x = x_seq, y = rep(price.range.tab$min[3],2), type = "scatter", mode = "lines",
            line     = list(color = PAL_FRAC[3], width = 0.5 ), showlegend=F) |> 
  add_trace(x = x_seq, y = rep(price.range.tab$max[3],2), type = "scatter", mode = "lines", fill = 'tonexty', fillcolor = "#7570b320",  
            line = list(color = PAL_FRAC[3], width = 0.5), showlegend=T, name="Price range Residues (P3)")
...
fig_uc %>%
  layout(
    xaxis = list(
      title    = "Aver. distance to customer (km)",
      zeroline = FALSE,
      dtick    = 100
    ),
    yaxis = list(
      title     = "Unit cost SC<sub>p,i</sub> (€/t)"
    ),
    ...
    width    = 1000,
    height   = 500#,
    #margin   = list(l = 55, r = 140, t = 20, b = 00)
  )
```
...
:::: {.callout-note icon="false" appearance="simple"}
::: {style="font-size: 1.8rem;"}
$c^{\text{opp}}_i$ can be negative (subsidies / positive crop-yield spillovers) or positive (high-value arable land).
:::
::::
