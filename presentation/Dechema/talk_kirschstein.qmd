...
```{r , echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=4.25, out.width="99%", cache=TRUE}
...
```{r , echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=5, out.width="99%", cache=TRUE}
...
vol_site_frac <- ext$Xij %>%
  group_by(site_id, p) %>%
  summarise(vol_raw_t = sum(value, na.rm = TRUE), .groups = "drop") %>%
  ungroup() %>%
  group_by(site_id) %>%
...
# ── 5. Zusammenfassung: Median-Stückkosten je Fraktion ────────────────────────
unit_cost_summary <- unit_costs %>%
  group_by(fraction, product) %>%
  summarise(
    n_sites        = n(),
    median_c_prod  = median(c_prod,       na.rm = TRUE),
    median_c_tr    = median(c_tr,         na.rm = TRUE),
    median_c_unit  = median(c_unit_total, na.rm = TRUE),
    .groups = "drop"
  )
...
unit_cost_summary %>%
  ...
  knitr::kable(
    caption = "Stückkosten je Fraktion (Median über alle aktiven Standorte)",
    col.names = c("Fraktion", "p", "n Sites",
                  "c_prod (€/t)", "c_tr (€/t)", "SC (€/t)"
                  )
  )
# ══════════════════════════════════════════════════════════════════════════════
# AUSGABE-OBJEKTE (für nachfolgende Chunks verfügbar):
#   prod_costs        – 1 Zeile/Standort, alle Kostenkomponenten absolut + €/ha/a
#   prod_costs_long   – long-format für Stacked-Bar-Charts
#   prod_costs_frac   – Fraktionsallokation: c_prod_p je Standort × Fraktion (€/t)
#   alloc_weights     – Allokationsgewichte w_p je Fraktion
# ══════════════════════════════════════════════════════════════════════════════
```


```{r}
#| label: fig-unit-costs-scatter
#| fig-width: 9
#| fig-height: 5.5
#| out-width: "99%"
#| echo: false
...
for (frac in levels(unit_costs$fraction)) {
  ...
  if (nrow(df_f) >= 3) {
    lm_f  <- lm(c_unit_total ~ avg_dist_km, data = df_f, weights = df_f$vol_raw_t)
    x_seq <- seq(min(df_f$avg_dist_km, na.rm = TRUE),
                 max(df_f$avg_dist_km, na.rm = TRUE),
                 length.out = 80)
    y_hat <- predict(lm_f, newdata = data.frame(avg_dist_km = x_seq))
...
x_seq <- round(c(min(unit_costs$avg_dist_km, na.rm = TRUE), max(unit_costs$avg_dist_km, na.rm = TRUE)))

fig_uc <- fig_uc |> 
  add_trace(x = x_seq, y = rep(price.range.tab$min[1],2), type = "scatter", mode = "lines" ,
            line     = list(color = PAL_FRAC[1], width = 0.5 ), showlegend=F) |> 
  add_trace(x = x_seq, y = rep(price.range.tab$max[1],2), fill = 'tonexty', fillcolor = "#1b9e7720",  line = list(color = PAL_FRAC[1], width = 0.5), showlegend=T, name="Price range Stems (P1)")
  
fig_uc <- fig_uc |> 
  add_trace(x = x_seq, y = rep(price.range.tab$min[2],2), type = "scatter", mode = "lines",
            line     = list(color = PAL_FRAC[2], width = 0.5 ), showlegend=F) |> 
  add_trace(x = x_seq, y = rep(price.range.tab$max[2],2), fill = 'tonexty', fillcolor = "#d95f0220",  line = list(color = PAL_FRAC[2], width = 0.5), showlegend=T, name="Price range Branches (P2)")

fig_uc <- fig_uc |> 
  add_trace(x = x_seq, y = rep(price.range.tab$min[3],2), type = "scatter", mode = "lines",
            line     = list(color = PAL_FRAC[3], width = 0.5 ), showlegend=F) |> 
  add_trace(x = x_seq, y = rep(price.range.tab$max[3],2), fill = 'tonexty', fillcolor = "#7570b320",  line = list(color = PAL_FRAC[3], width = 0.5), showlegend=T, name="Price range Residues (P3)")
...
fig_uc %>%
  layout(
    ...
    width    = 880,
    height   = 480,
    margin   = list(l = 55, r = 140, t = 20, b = 80)
  )
```
