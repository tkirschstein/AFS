# ============================================================================
# SPARSE LP BUILDER FOR AGROFORESTRY SCD PROBLEM
# Stand-alone function: build_agroforestry_lp_sparse()
# ============================================================================
# 
# USAGE:
#   built <- build_agroforestry_lp_sparse(instance)
#   result <- solve_agroforestry_sparse(instance, time_limit=600)
#
# DEPENDENCIES:
#   library(Matrix)
#   library(ROI)
...
build_agroforestry_lp_sparse_v2 <- function(instance) {
  ...
  Y_tuples <- expand.grid(i = I, p = Pset, t = Tset)
  Y_tuples$col <- n_z + seq_len(nrow(Y_tuples))
  n_Y <- nrow(Y_tuples)
  
  # X_ij[i, j, p, t] - flows site to hub
  Xij_tuples <- expand.grid(i = I, j = J, p = Pset, t = Tset)
  Xij_tuples$col <- max(Y_tuples$col) + seq_len(nrow(Xij_tuples))
  n_Xij <- nrow(Xij_tuples)
  
  # X_jk[j, k, p, t] - flows hub to consumer
  Xjk_tuples <- expand.grid(j = J, k = K, p = Pset, t = Tset)
  Xjk_tuples$col <- max(Xij_tuples$col) + seq_len(nrow(Xjk_tuples))
  n_Xjk <- nrow(Xjk_tuples)
  
  # S_jpt[j, p, t] - inventory at storage hubs
  S_tuples <- expand.grid(j = J, p = Pset, t = Tset)
  S_tuples$col <- max(Xjk_tuples$col) + seq_len(nrow(S_tuples))
  n_S <- nrow(S_tuples)
  
  n_vars <- max(S_tuples$col)
  
  cat(sprintf("  Total variables: %d (z:%d Y:%d Xij:%d Xjk:%d S:%d)\n",
              n_vars, n_z, n_Y, n_Xij, n_Xjk, n_S))
  ...
  # Revenue from shipments to consumers: R_p[p] * X_jk[j,k,p,t]
  ...
  for (i in I) {
    for (p in Pset) {
      for (t in Tset) {
        if (t == 1) {
          # Y_{i,p,1} = 0 (no harvest in first period)
          Y_col <- Y_tuples$col[Y_tuples$i == i & Y_tuples$p == p & Y_tuples$t == 1]
          if (length(Y_col) > 0) {
            
            const_count <- const_count + 1L
            row_idx <- c(row_idx, const_count)
            col_idx <- c(col_idx, Y_col)
            val     <- c(val, 1)
            sense_vec <- c(sense_vec, "==")
            rhs_vec <- c(rhs_vec, 0)
  ...
  # S_{j,p,t} = S_{j,p,t-1} + sum_i X_{ijpt} - sum_k X_{jkpt}
  for (j in J) {
    for (p in Pset) {
      # t = 1
      ...
      X_in_1 <- Xij_tuples$col[Xij_tuples$j == j & Xij_tuples$p == p & 
                                 Xij_tuples$t == 1]
      X_out_1 <- Xjk_tuples$col[Xjk_tuples$j == j & Xjk_tuples$p == p & 
                                  Xjk_tuples$t == 1]
      ...
      for (t in Tset[Tset >= 2]) {
        ...
        X_in_t <- Xij_tuples$col[Xij_tuples$j == j & Xij_tuples$p == p & 
                                   Xij_tuples$t == t]
        X_out_t <- Xjk_tuples$col[Xjk_tuples$j == j & Xjk_tuples$p == p & 
                                    Xjk_tuples$t == t]
  ...
  # sum_j sum_{q <= p} X_{jkqt} >= D_max(k,p,t)
  for (k in K) {
    for (p in Pset) {
      for (t in Tset) {
        # Find demand
...
# OP with consumer-specific revenues per product #############
build_agroforestry_lp_sparse_v3 <- function(instance) {
  ...
  # D_kpt - demand data frame
  Dkpt_tuples <- expand.grid(k = K, p = Pset, t = Tset)
  Dkpt_tuples$col <- max(S_tuples$col) + seq_len(nrow(Dkpt_tuples))
  n_Dkpt <- nrow(Dkpt_tuples)
  
  n_vars <- max(Dkpt_tuples$col)
  
  cat(sprintf("  Total variables: %d (z:%d Y:%d Xij:%d Xjk:%d S:%d D:%d)\n",
              n_vars, n_z, n_Y, n_Xij, n_Xjk, n_S, n_Dkpt))
  ...
  # rename columns
  colnames(consumer_prices) <- c("k", "p", "price")
  
  # find prices for each (k,p) pair
  revenue.vec <- Dkpt_tuples %>%
    left_join(consumer_prices, by = c("k", "p")) %>%
    mutate(col_idx = col) %>%
    select(col_idx, price) 
  
  # Revenue from shipments to consumers: R_p[k,p] * Dkpt[k,p,t]
  ...
  for (k in K) {
    for (p in Pset) {
      for (t in Tset) {
        ...
        D_max_kpt <- dem_row$D_max[1]
        # find variable D_kpt
        D_cols <- Dkpt_tuples$col[Dkpt_tuples$k == k & Dkpt_tuples$p == p & 
                                    Dkpt_tuples$t == t]
        # Get X_jk variables with product q <= p
        X_cols <- Xjk_tuples$col[
          Xjk_tuples$k == k &
        ...
        if (length(X_cols) > 0 && length(D_cols) > 0) {
          # shipment constraint
          const_count <- const_count + 1L
          row_idx <- c(row_idx, rep(const_count , length(X_cols)+length(D_cols))) 
          col_idx <- c(col_idx, X_cols, D_cols)
          val     <- c(val, rep(1, length(X_cols)), rep(-1, length(D_cols))) 
          sense_vec <- c(sense_vec, "==")
          rhs_vec <- c(rhs_vec, 0)
          
          # demand constraint
          const_count <- const_count + 1L
          row_idx <- c(row_idx, rep(const_count , length(D_cols)))
          col_idx <- c(col_idx, D_cols)
          val     <- c(val, rep(1, length(D_cols)))
          sense_vec <- c(sense_vec, "<=")
          rhs_vec <- c(rhs_vec, D_max_kpt)
  ...
  model <- OP(
    ...
    types      = c(rep("B", n_z),
                   rep("C", n_Y + n_Xij + n_Xjk + n_S + n_Dkpt))
  )
...
# OP with opportunity cost and product-specific demands #############
build_agroforestry_lp_sparse_v4 <- function(instance) {
  ...
  # set ub to 0 for s >0 & ((t-s) < Amin or (t-s) > Amax) 
  ...
  Xjk_tuples <- expand.grid(j = J, k = K, p = Pset, pp = Pset, t = Tset)
...
# ============================================================================
# FUNCTION: Solve the LP with Gurobi
# ============================================================================
...
solve_agroforestry_sparse <- function(instance,
  ...
  cat("========================================================\n")
  cat("  AGROFORESTRY SCD - SPARSE LP SOLVER\n")
  cat("========================================================\n\n")
  
  # Build LP
  #built <- build_agroforestry_lp_sparse_v4(instance)
  built <- build_agroforestry_lp_sparse_v10(instance)
  # Solve with Gurobi via ROI
  #cat("Solving with Gurobi...\n")
  if(solver == "gurobi"){
    result <- ROI_solve(
      built$model,
      solver = "gurobi", 
...
extract_result <- function(opt_result){
  ...
  # harvest quantities
  Y_solution <- opt_result$solution$Y
  
  Y_solution_filtered <- Y_solution %>% 
    filter(value > 0.01) %>%
    arrange(i, t, p)
