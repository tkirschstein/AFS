build_agroforestry_lp_sparse_v10_optimized <- function(instance) {
  ...
  cat("Building OPTIMIZED sparse LP for agroforestry SCD problem (MILP v10)...\n")
  ...
  # Y[ii,pprod,tt]
  Y_tuples <- as.data.table(expand.grid(ii = I, pprod = Pset, tt = Tharv))
  Y_tuples[, ub  := area_ha[as.character(ii)] * yield_max[pprod]]
  Y_tuples[, col := .I + n_z]
  n_Y <- nrow(Y_tuples)
  ...
  for (ii_idx in I) {
    z_est_i <- z_tuples[.(ii_idx, 0, Tset), nomatch = 0]
    if (nrow(z_est_i) > 0) {
  ...
  ## C3: Biomass yield (Y only in Tharv)
  for (ii_idx in I) {
    area_i <- area_ha[ii_idx]
    for (pprod in Pset) {
      for (tt_idx in Tharv) {
        Y_row  <- Y_tuples[.(ii_idx, pprod, tt_idx), nomatch = 0]
        if (nrow(Y_row) == 0) next
        Y_col  <- Y_row$col
        z_arcs <- z_tuples[.(ii_idx)][t == tt_idx & s >= 1 & s < tt_idx]
        if (nrow(z_arcs) > 0) {
  ...
  for (ii_idx in I) {
    for (p in Pset) {
      for (tt_idx in Tharv) {
        Y_row  <- Y_tuples[.(ii_idx)][pprod == p & tt == tt_idx]
        if (nrow(Y_row) == 0) next
        Y_col  <- Y_row$col
        X_cols <- Xij_tuples[.(ii_idx)][pprod == p & tt == tt_idx]$col
        if (length(X_cols) > 0) {
  ...
  for (jj_idx in J) {
    for (p in Pset) {
      ...
      for (tt_idx in Tharv[Tharv >= Amin+1]) {
        ...
        X_out_t   <- Xjk_tuples[.(jj_idx)][pprod == p & pp >= p & tt == tt_idx]$col
  ...
  for (jj_idx in J) {
    for (tt_idx in Tharv) {
      S_cols <- S_tuples[.(jj_idx)][tt == tt_idx]$col
      if (length(S_cols) > 0) {
  ...
  cat("OPTIMIZED MILP v10 OP model built successfully!\n\n")
